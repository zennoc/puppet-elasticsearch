...
# - Set variables (at top scope level on in a ENC) and "include elasticsearch"
# - Call elasticsearch as a parametrized class
#
...
class elasticsearch (
  ...
  $heap_size	       = params_lookup( 'heap_size' )
  ...
  service { 'elasticsearch':
    ...
    require    => Package[$elasticsearch::package],
  }
  ...
  if $elasticsearch::source_dir {
    ...
    file { 'default-elasticsearch':
      ...
      content	=> template('elasticsearch/default.elasticsearch'),
      mode	=> $elasticsearch::config_file_mode,
      owner	=> $elasticsearch::config_file_owner,
      group	=> $elasticsearch::config_file_group,
      notify	=> $elasticsearch::manage_service_autorestart,
